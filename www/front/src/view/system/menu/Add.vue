...
const menuRef = ref();
const rules = {
  type_id: [{ required: true, message: "菜单类型不能为空", trigger: "blur" }],
 ...
 */
const init = async () => {
  const response = await axios.get("manage/system.Menu/initAdd");
  if (response.code != 1) {
    ElMessage({
...
const submitForm = () => {
  menuRef.value.validate(async (valid) => {
    ...
    const response = await axios.post("manage/system.Menu/saveAdd", menu.value);
